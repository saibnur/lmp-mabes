...
export default function EditBeritaPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (profileLoading || isLoading) return <div className="p-8 text-center pt-32">Memuat...</div>;
    ...
    if (!article) return <div className="p-8 text-center pt-32">Artikel tidak ditemukan</div>;

    // Check permissions
    if (article.authorId !== uid && profile.role !== 'admin') {
        if (typeof window !== 'undefined') {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // If navigating back to detail page, we can route to: router.push(`/berita/${id}`);
            // Let's assume we navigate back to the article detail (which we'll build shortly)
    ...
    return (
        <div className="bg-slate-50 min-h-screen pt-24 pb-20">
            <div className="container mx-auto px-4 max-w-4xl">
                <div className="mb-6">
                    <button onClick={() => router.back()} className="inline-flex items-center text-sm font-bold text-slate-500 hover:text-slate-800 transition">
