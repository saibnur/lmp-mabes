 ...
 * Fetch regional data from Emsifa API (Proxy)
 */
router.get('/regions', async (req, res) => {
    ...
    try {
        ...
        console.error(`Error fetching ${type} from API:`, error.message);
        res.status(500).json({ success: false, message: 'Gagal mengambil data wilayah dari server' });
    }
});

/**
 * POST /api/members/update-profile
 * Update profile and generate KTA automatically
 */
router.post('/update-profile', verifyToken, async (req, res) => {
    ...
    try {
        const updatedUser = await db.runTransaction(async (transaction) => {
            ...
            // Logic: Hanya generate KTA jika belum ada
            if (!no_kta) {
                const village_id = organization.village_id;

                // Query Ambil Nomor KTA Terakhir di Desa Tersebut
                // Peringkat: village_id ASC, no_kta DESC (Requirement Composite Index)
                ...
                // Format village_id: 1101012001 -> 11.01.01.2001
                const v = village_id;
        ...
        console.error('Update profile transaction error:', error);
